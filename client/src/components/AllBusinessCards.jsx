        ...
        <div style={{ marginBottom: "100px" }}>
            ...
            {loading ? <Spinner className='loadingSpinner' style={{ top: '300px' }} animation="grow" variant="secondary" /> :
                <motion.div
                    ...
                    transition={{ duration: .8 }}
                    id='cardContiner'>
                    {tokenExpired ? <div className="messageToClient"><h4 style={{ fontSize: '2em' }}>Dear customer,<br /> Your session is expired, you can log in again by <span style={{ cursor: 'pointer', color: '#528265', textShadow: '1px 1px 1px #6f6f6f' }} onClick={() => tokenExpiredHandle()}>clicking here</span></h4></div> :
                        itemsArr.length === 0 ?
                                    ...
                                    <Card sx={{ width: 260, height: 420 }} key={item._id} id='card'>
                                        ...
                                        <CardContent className="cardContent">
                                            ...
                                            <Typography variant="body2" color="text.secondary">
                                                <GoLocation style={{ color: 'black', fontWeight: '900' }}>Address:</GoLocation> {item.businessAddress}
                                            </Typography>
                                            <Typography variant="body2" color="text.secondary">
                                                <BsTelephone style={{ color: 'black', fontWeight: '900' }}>Tel:</BsTelephone> {item.businessPhoneNumber}
                                            </Typography>
                                            <Typography className="cardDate" variant="body2" color="text.secondary">
                                                <AiOutlineFieldTime style={{ color: 'black', fontWeight: '900' }}>Date:</AiOutlineFieldTime> {item.createDate.slice(0, 10)}
                                            </Typography>
                                        </CardContent>
                                    </Card>
                                )
